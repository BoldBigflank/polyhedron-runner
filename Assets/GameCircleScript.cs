...
public class GameCircleScript : MonoBehaviour {
	...
	void Update () {
		if(!didScore15 && GameManager.score == 15){
			didScore15 = true;
			ReportAchievement ("score_15", 1.0F);
		}

		if(!didScore30 && GameManager.score == 30){
			didScore30 = true;
			ReportAchievement("score_30", 1.0F);
		}
		if(!didScore50 && GameManager.score == 50){
			didScore50 = true;
			ReportAchievement ("score_50", 1.0F);
		}
		
		if(!didScore75 && GameManager.score == 75){
			didScore75 = true;
			ReportAchievement ("score_75", 1.0F);
		}
		
		if(!didScore100 && GameManager.score == 100){
			didScore100 = true;
			ReportAchievement("score_100", 1.0F);
		}
	}
	...
	void GameOver(){ // Called by the HollowCubeScript
		...
		if(dataMap != null){ 
			...
			if(gamesPlayed.AsInt() <= 10){
				ReportAchievement ("play_10", gamesPlayed.AsInt() / 10.0F  );
			}
			if(gamesPlayed.AsInt() <= 50){
				ReportAchievement ("play_50", gamesPlayed.AsInt() / 50.0F  );
			}
			if(gamesPlayed.AsInt() <= 100){
				ReportAchievement ("play_100", gamesPlayed.AsInt() / 100.0F  );
			}
			if(gamesPlayed.AsInt() <= 500){
				ReportAchievement ("play_500", gamesPlayed.AsInt() / 500.0F  );
			}
			if(gamesPlayed.AsInt() <= 1000){
				ReportAchievement ("play_1000", gamesPlayed.AsInt() / 1000.0F  );
			}
		}
	}
	...
	private void ReportScore(int score){
		if(isServiceReady){
			...
			if(score == 0) ReportAchievement("score_0", 1.0F);
		}else{
